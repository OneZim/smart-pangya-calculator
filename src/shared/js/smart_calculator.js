...
function calculeTypeDistance(distance) {
  ...
  else if (distance < 10.0) return TYPE_DISTANCE.LESS_10;
  else if (distance < 15.0) return TYPE_DISTANCE.LESS_15;
  else if (distance < 28.0) return TYPE_DISTANCE.LESS_28;
  else if (distance < 58.0) return TYPE_DISTANCE.LESS_58; // Esse não precisa já que ele passou do primeiro if, mas deixar assim

  return type;
}

function calculeTypeDistanceByPosition(position1, position2) {
  return calculeTypeDistance(
    Math.sqrt(
      Math.pow(position1.x - position2.x) + Math.pow(position1.z - position2.z),
    ) * 0.3125,
  );
}
...
const CLUB_INFO = {
  ...
  _2W: new ClubInfo(CLUB_TYPE.WOOD, 0.5, 1.41, 204.0, 13.0, 210.0),
  ...
  _8I: new ClubInfo(CLUB_TYPE.IRON, 0.3, 0.57, 114.0, 44.0, 120.0),
  ...
  PT1: new ClubInfo(CLUB_TYPE.PT, 0.0, 0.0, 30.0, 0.0, 20.0),
  PT2: new ClubInfo(CLUB_TYPE.PT, 0.0, 0.0, 21.0, 0.0, 10.0),
};
...
const SHOT_TYPE_ENUM = ["DUNK", "TOMAHAWK", "SPIKE", "COBRA"];
...
class Club {
  ...
  getPower(extraPower, pwrSlot, ps, spin) {
    ...
    switch (this.type) {
      case CLUB_TYPE.WOOD: {
        pwrjard =
          extraPower.total(ps) + getPowerShotFactory(ps) + (pwrSlot - 15) * 2;
      ...
      case CLUB_TYPE.IRON: {
        pwrjard =
          (getPowerShotFactory(ps) / this.power_base + 1.0) *
            this.power_factor +
      ...
      case CLUB_TYPE.PW: {
        ...
        const getPowerByDegree = (degree, spin) => {
          return (
            0.5 +
            (0.5 * (degree + spin * _00D19B98)) / ((56 /*Ang*/ / 180) * Math.PI)
          );
        };
  ...
  getPower2(extraPower, pwrSlot, ps) {
    let pwrjard =
      (extraPower.auxpart + extraPower.mascot + extraPower.card) / 2 +
      (pwrSlot - 15);

    if (ps) pwrjard += extraPower.ps_card / 2;
  ...
  getRange(extraPower, pwrSlot, ps) {
    let pwr_range =
      this.power_base + extraPower.total(ps) + getPowerShotFactory(ps);

    if (this.type == CLUB_TYPE.WOOD) pwr_range += (pwrSlot - 15) * 2;
...
class Wind {
  ...
  getWind() {
    return new Vector3D(
      this.wind * Math.sin((this.degree * Math.PI) / 180) * -1,
      0,
      this.wind * Math.cos((this.degree * Math.PI) / 180),
    );
  }
}
...
class QuadTree {
  ...
  initShot(ball, club, wind, options) {
    ...
    this.power_range_shot = this.club.getRange(
      options.power.options,
      options.power.pwr,
      options.ps,
    );
    ...
    if (
      options.shot == SHOT_TYPE.TOMAHAWK ||
      options.shot == SHOT_TYPE.SPIKE /*Toma e spike */
    )
      pwr *= 1.3;
    else pwr *= 1.0;
    ...
    let value1 = this.getValuesDegree(
      options.mira_rad + (0 - this.ball.curva * _00D17908),
      1,
    );
    let value2 = this.getValuesDegree(
      this.club.type_distance == TYPE_DISTANCE.BIGGER_OR_EQUAL_58
        ? this.club.getDregRad()
        : this.club.getDregRad() + this.ball.spin * _00D19B98,
      0,
    );

    this.ball.curva -= this.getSlope(
      options.mira_rad - options.slope_mira_rad,
      Math.random() /* Rotation Ball Line */,
    );
    ...
    this.ball.velocity.x =
      v2.x * vectA.y + vectA.x * v1.x + v3.x * vectA.z + v4.x;
    this.ball.velocity.y =
      v1.y * vectA.x + v2.y * vectA.y + v3.y * vectA.z + v4.y;
    this.ball.velocity.z =
      v1.z * vectA.x + v2.z * vectA.y + v3.z * vectA.z + v4.z;

    // Rotação eixo X, Z
    this.ball.rotation_curve = this.ball.curva * options.percentShot;
    this.ball.rotation_spin =
      this.club.type_distance == TYPE_DISTANCE.BIGGER_OR_EQUAL_58
        ? this.club.getPower2(
            options.power.options,
            options.power.pwr,
  ...
  getSlope(mira, line_ball) {
    ...
    // Calc slope
    let ball_slope_cross_const_vect = this.ball.slope
      .clone()
      .cross(_00E42544_vect_slope);

    let slope_matrix = {
      v1: ball_slope_cross_const_vect.clone().normalize(),
      v2: this.ball.slope.clone(),
      v3: ball_slope_cross_const_vect
        .clone()
        .cross(this.ball.slope)
        .normalize(),
  ...
  ballProcess(steptime, final = undefined) {
    ...
    if (this.shot == SHOT_TYPE.COBRA && this.cobra_init < 0) {
      ...
      let diff = this.ball.position
        .clone()
        .sub(this.ball_position_init)
        .length();
      let cobra_init_up =
        (this.power_range_shot * this.percentShot_sqrt - 100.0) * 3.2;
      ...
      if (diff >= cobra_init_up) {
        ...
        this.ball.velocity
          .normalize()
          .multiplyScalar(power_multiply)
          .multiplyScalar(this.percentShot_sqrt);
      ...
      if (
        ...
        this.ball.rotation_spin -=
          (_00D66CA0 - this.ball.spin * _00CFF040) * _00D083A0;
      } else if (
        (this.shot == SHOT_TYPE.SPIKE && this.spike_init >= 0) ||
        (this.shot == SHOT_TYPE.COBRA && this.cobra_init >= 0)
      )
      ...
      if (this.shot == SHOT_TYPE.SPIKE && this.ball.count == 0) {
        ...
        this.ball.velocity
          .normalize()
          .multiplyScalar(72.5)
          .multiplyScalar(this.percentShot_sqrt * 2);
  ...
  bounceProcess(steptime, final = undefined) {
    ...
    if (this.ball.num_max_height == -1) {
      let tmpVect = this._21D8_vect
        .clone()
        .divideScalar(this.ball.mass)
        .multiplyScalar(steptime);
    ...
    this.ball.position.add(
      this.ball.velocity
        .clone()
        .multiplyScalar(final !== undefined ? final : steptime),
    );
  }
  ...
  applyForce() {
    ...
    if (this.ball.rotation_curve != 0) {
      let vectorb = new Vector3D(
        this.ball.velocity.z * _00D046A8,
        0,
        this.ball.velocity.x,
      );
      ...
      if (this.cobra_init < 0 || this.spike_init < 0)
        vectorb.multiplyScalar(
          _00D00190 * this.ball.rotation_curve * this.club.rotation_curve,
        );
    ...
    windVect.multiplyScalar(
      this.shot == SHOT_TYPE.SPIKE ? _00D16758 : _00D083A0,
    );
    ...
    if (this.ball.rotation_spin != 0)
      retVect.y =
        retVect.y +
        this.club.rotation_spin * _00D66CF8 * this.ball.rotation_spin;
...
const diffYZ = function (vect1, vect2) {
  return Math.sqrt(
    Math.pow(vect1.x - vect2.x, 2) + Math.pow(vect1.z - vect2.z, 2),
  );
};
...
const find_power = (
  ...
  // init Club Info
  if (club != undefined) vclub.init(club_info);
  ...
  if (slope instanceof Vector3D) {
    ...
    vball.slope = new Vector3D(
      slope * slope_break_to_curve_slope * -1,
      1.0,
      0.0,
    );
  ...
  const powerRange = vclub.getRange(
    options.power.options,
    options.power.pwr,
    options.ps,
  );
  ...
  const findAlturaColision = (qt, altura_colision) => {
    ...
    do {
      ...
      (vball.position.y > altura_colision || vball.num_max_height == -1) &&
      count++ < 3000
    );

    let last_step = Math.abs(
      (altura_colision - copy_ball.position.y) /
        (vball.position.y - copy_ball.position.y),
    );
    ...
    if (Math.abs(distanciaScale - vball.position.z) <= margin) return 0;
  ...
  do {
    ...
    if (options.percentShot > 1.3) options.percentShot = 1.3;
    else if (options.percentShot < 0.0) options.percentShot = 0.1;

    qt.initShot(vball, vclub, wind, options);

    ret = findAlturaColision(qt, altura_colision);

    if (ret == 0) isFind = true;
    else {
      ...
      if (lado == 0) lado = ret < 0 ? -1 : 1;
      else if ((ret < 0 && lado == 1) || (ret > 0 && lado == -1)) feed *= 0.5;
  ...
  if (isFind) {
    ...
    found.desvio =
      (vball.position.x +
        (options.position.x + Math.tan(options.mira_rad) * distanciaScale)) *
      DESVIO_SCALE_PANGYA_TO_YARD;
    found.power_range = powerRange;
    found.smartData = {
      desvio: found.desvio,
      altura: altura_colision,
      club: vclub,
      options: options,
    };
  }
...
function calc(el) {
  ...
  // 🔴 DÉFINIR DUNK RESULT AVANT DE L'UTILISER
  // 🎯 Optimiseur de spin pour l'overlay (reflète le bon algo selon le shot)
  let dunkResult = null;
  try {
    if (shot === SHOT_TYPE.DUNK && typeof findBestDunkSpin === "function") {
      dunkResult = findBestDunkSpin(input_values, {
        tolerance: 0.2,
        allowHardSpins: true,
      });
    } else if (
      (shot === SHOT_TYPE.TOMAHAWK || shot === SHOT_TYPE.SPIKE) &&
      typeof findBestTomahawkSpikeSpin === "function"
    ) {
      dunkResult = findBestTomahawkSpikeSpin(input_values);
    }
    // Cobra : dunkResult reste null, pas d'optimisation
  } catch (e) {
    console.error("❌ Erreur dans l'optimiseur de spin:", e);
  }
  ...
  if (f[index_f].power != -1) {
    ...
    // Fallback immédiat
    result.innerHTML =
      "Le tir n’a pas pu être calculé, il est trop court ou trop faible.";

    // Si la langue est déjà chargée → traduction immédiate
    if (window.i18nReady) {
      result.innerHTML = t("error_shot_too_short");
    }

    // Sinon → attendre que Tauri charge la langue
    document.addEventListener("i18n-loaded", () => {
      result.innerHTML = t("error_shot_too_short");
    });
  }

  //Ajout pour Taur i et l'app on recupere la disatnce et le PB final
  if (f[index_f].power != -1) {
    // 🔴 On ajoute juste les variables et l'envoi Tauri ici, sans toucher à ton affichage
    const pbValue =
      parseFloat(
        (desvioByDegree(f[index_f].desvio, distance) / 0.2167).toFixed(2),
      ) || 0.0;
    ...
    const displayPercent =
      parseFloat((f[index_f].power * 100).toFixed(1)) || 0.0;

    // 🆕 Meilleur spin/capler pour dunker sur cette distance déjà corrigée (vent + hauteur)

    if (window && window.__TAURI__ && window.__TAURI__.event) {
      window.__TAURI__.event.emit("update-ruler", {
        pb: pbValue,
        distance: distanceYards,
        percent: displayPercent,
        recommendedSpin: dunkResult ? dunkResult.spin : null,
        recommendedCapler: dunkResult ? dunkResult.capler : null,
      });
    }
  } else {
    // 🔴 Envoi de "0" si le tir n'est pas calculable
    if (window && window.__TAURI__ && window.__TAURI__.event) {
      window.__TAURI__.event.emit("update-ruler", {
        pb: null,
        distance: null,
...
function calcMycella(el) {
  const shot_degree = checkValidInput(
    document.querySelector("#mycella>#shot-degree").value,
  );
  const align_degree = checkValidInput(
    document.querySelector("#mycella>#align-degree").value,
  );
  const slope_break = checkValidInput(
    document.querySelector("#mycella>#slope-break").value,
  );

  const slope_real =
    Math.abs(
      Math.cos(Math.abs((Math.PI / 180) * (shot_degree - align_degree))),
    ) * slope_break;

  document.getElementById("degree").value = shot_degree;
  document.getElementById("slope_break").value = (
    slope_real / getSlopeByResolution()
  ).toFixed(3);
}
...
function getResolutionPBLimit() {
  ...
  let value =
    ((480 / resolution.height) * 0.006 * (resolution.width / 2)) / YARDS_TO_PB;
...
function smartDesvio(smartData) {
  ...
  if (Math.abs(pb_sample) <= MAX_PB) return `${pb_sample.toFixed(2)}pb`;

  pb_sample = yards / YARDS_TO_PBA;

  if (Math.abs(pb_sample) <= MAX_PB) return `${pb_sample.toFixed(2)}pba`;
  ...
  for (let i = 12; i >= 0; --i) {
    ...
    powerRange = smartData.club.getRange(
      smartData.options.power.options,
      smartData.options.power.pwr,
      smartData.options.ps,
    );

    pb_sample =
      yards /
      YARDS_TO_PB /
      ((powerRange * 3.2 * 1.4 - smartData.altura) * 0.0625);
