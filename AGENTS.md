...
- **Frontend (`src/`)** : JavaScript vanilla (sans bundler), structuré par dossiers :
  - `src/index.html` + `src/app.js` : fenêtre principale (configuration, toggles des overlays).
  - `src/core/` : `components/`, `services/`, `stores/` (Services : `TauriService`, `StorageService`, `ResolutionCalibrationService`, `CourseService`).
  - `src/models/`, `src/shared/` (JS et CSS partagés, thèmes), `src/style/`, `src/utils/` (`WindowPositionHelper`, `WindowDragHelper`, `recalc.js`, `dunk_otions_boutons.js`).
...
## 🛠️ Règles Strictes pour l'Agent (Claude Sonnet 5)
