...
fn move_and_click_focused(x: f64, y: f64) -> Result<(), String> {
    // 1. Laisser le temps au clic de l'utilisateur sur l'overlay de se terminer proprement
    std::thread::sleep(std::time::Duration::from_millis(100));

    let mut enigo = Enigo::new(&Settings::default()).map_err(|e| e.to_string())?;

    // 2. Déplacer la souris vers la cible (le cadran de spin du jeu) AVANT d'activer la fenêtre.
    // Comme ça, s'il y a un clic focus résiduel, il se fera directement au bon endroit.
    enigo
        .move_mouse(x.round() as i32, y.round() as i32, Coordinate::Abs)
        .map_err(|e| e.to_string())?;
    ...
    // 4. Effectuer le clic sur le cadran de spin
    enigo
...
fn move_spin_overlay(app_handle: tauri::AppHandle, dx: i32, dy: i32) -> Result<(), String> {
    if let Some(window) = app_handle.get_webview_window("spin_overlay") {
        let mut pos = window.outer_position().map_err(|e| e.to_string())?;
        pos.x += dx;
        pos.y += dy;
        window.set_position(pos).map_err(|e| e.to_string())?;
...
fn set_infos_shot_visibility(app: AppHandle, show: bool) {
    if let Some(win) = app.get_webview_window("infos_shot") {
        if show {
            let _ = win.show();
            reapply_click_through(&app, "infos_shot");
        } else {
            let _ = win.hide();
        }
    }
    let _ = app.emit("sync-infos-shot-visibility", show);
}
...
pub fn run() {
    ...
    tauri::Builder::default()
        ...
        .setup(|app| {
            ...
            enable_transparency(handle, "input_overlay");
            enable_transparency(handle, "spin_overlay");
            enable_transparency(handle, "infos_shot");
